...
def _check_backend(fix: bool) -> tuple[int, int, int, int, str]:
    ...
    reformat_files = {
        ...
        if "would reformat" in line.lower() and len(line.split()) > 1
    }
...
def _check_frontend(fix: bool) -> tuple[int, int, int, int, str]:
    ...
    eslint_cmd = ["npx", "next", "lint", "--dir", "app", "--dir", "components", "--dir", "lib"]
    if fix:
    ...
    eslint_files = {
        ...
        if line.strip().startswith(("./", "app/", "components/", "lib/"))
    }

    all_files = prettier_files | eslint_files
    total_files = len(all_files)

    warning_count = len(prettier_files) + sum(
        1 for line in eslint_lines if "warning" in line.lower() or "Warning:" in line
    )
    error_count = sum(
        1 for line in eslint_lines if "error" in line.lower() or "Error:" in line
    )
...
def _format_status(name: str, files: int, errors: int, warns: int) -> str:
    ...
    return f"{name:<9} {file_str} {verb} improvement {detail_str}."
...
def run_linters(fix: bool = False, verbose: bool = False) -> int:
    ...
    status_lines = {
        "backend": "Backend:  Linting...",
        "frontend": "Frontend: Linting...",
    }
    ...
    try:
        if console:
            with Live(render_text(), console=console, refresh_per_second=10) as live:
    ...
    if py_code != 0 or fe_code != 0:
        print("\n▫ Run 'tmd lint -v' for verbose output.")
        print("▫ Run 'tmd lint --fix' to automatically fix issues.\n")
        return 1

    print("\n✓ All backend and frontend linter checks passed!")
    return 0
...
def main() -> None:
    ...
    parser.add_argument("-v", "--verbose", action="store_true", help="Show detailed verbose linter outputs.")
