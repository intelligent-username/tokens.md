...
def merge(
    ...
    console.print(
        f"[green]Merged[/green] {len(files)} file(s) -> {output_path.name} "
        f"[dim cyan](output ≈ {format_tokens(total_target)} tokens | saved ≈ {format_tokens(saved_tokens)} tokens [{pct:+.1f}%])[/dim cyan]"
    )
