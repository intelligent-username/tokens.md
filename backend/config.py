...
class Settings:
    ...
    cors_origins: list[str] = field(
        default_factory=lambda: list(DEFAULT_CORS_ORIGINS.split(","))
    )
    ...
    def from_env(cls) -> "Settings":
        ...
        return cls(
            ...
            session_ttl_hours=_env_int(
                "TMD_SESSION_TTL_HOURS", DEFAULT_SESSION_TTL_HOURS
            ),
            cors_origins=[
                origin.strip()
                for origin in os.environ.get(
                    "TMD_CORS_ORIGINS", DEFAULT_CORS_ORIGINS
                ).split(",")
                if origin.strip()
            ],
            ui_dir=_resolve_ui_dir(os.environ.get("TMD_UI_DIR")),
            allow_local_paths=_env_bool("TMD_ALLOW_LOCAL_PATHS", False),
            local_paths_root=Path(
                os.environ.get("TMD_LOCAL_PATHS_ROOT", str(Path.cwd()))
            ),
