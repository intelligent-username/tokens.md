...
export function ProgressBar({ value = 0, max = 100, indeterminate = false, label }: ProgressBarProps) {
  ...
  return (
    <div className="flex flex-col gap-1">
      <div role="progressbar" aria-label={label} aria-valuemin={0} aria-valuemax={max} aria-valuenow={indeterminate ? undefined : Math.round(value)} className="h-3 w-full overflow-hidden rounded-full bg-muted border border-border/40">
        {indeterminate ? (
          <motion.div className="h-full w-1/3 rounded-full bg-emerald-500" animate={reduced ? { x: 0 } : { x: ["-120%", "360%"] }} transition={{ duration: 1.2, repeat: Infinity, ease: "easeInOut" }} />
        ) : (
