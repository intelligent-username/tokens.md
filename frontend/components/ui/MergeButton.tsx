...
export function MergeButton({ label, onClick, loading = false, disabled = false }: MergeButtonProps) {
  ...
  return (
    <button
      ...
      disabled={busy}
      className={cn(
        "inline-flex h-10 items-center justify-center gap-2 rounded-control bg-primary px-4 font-sans text-sm font-bold text-zinc-950 shadow-glow transition-colors hover:bg-emerald-400 active:bg-emerald-600",
