...
export function FileMatrixRow({ file, fileKey, resultItem, meta, isConverting, isDone, mergeMode, sessionId, onRemoveFile, barRef }: FileMatrixRowProps) {
  return (
    <div key={fileKey} className={cn("relative grid items-center gap-2 sm:gap-4 rounded-card bg-card/60 p-3 sm:p-4 border border-border/60 hover:border-border transition-colors", mergeMode ? "grid-cols-[1fr_3fr]" : "grid-cols-[1fr_3fr_1fr]")}>
      ...
      <div className="flex items-center gap-2.5 min-w-0 min-h-[52px]">
        ...
        <span className={cn("h-2 w-2 shrink-0 rounded-full", isDone ? "bg-emerald-500" : isConverting ? "animate-pulse bg-amber-400" : "bg-muted-foreground/60")} />
        <div className="flex flex-col min-w-0">
          <span className="truncate font-mono text-xs sm:text-sm font-semibold text-foreground">{file.name}</span>
          <span className="font-mono text-[11px] text-muted-foreground">{formatBytes(file.size)}</span>
          {meta?.source_tokens ? <span className="font-mono text-[11px] text-muted-foreground">{formatTokens(meta.source_tokens)} tokens</span> : null}
        </div>
      </div>

      {mergeMode ? <div ref={barRef} className="h-3.5 w-full" /> : <FileFlowStream converting={isConverting} done={isDone} percent={resultItem?.percent} barRef={barRef} />}
      ...
      {!mergeMode ? (
        <div className="flex items-center justify-between gap-2 min-w-0 min-h-[52px]">
          {resultItem ? (
            <>
              <div className="flex flex-col min-w-0">
                <span className="truncate font-mono text-xs sm:text-sm font-semibold text-foreground">{resultItem.output_name ?? resultItem.name}</span>
                {resultItem.output_size ? <span className="font-mono text-[11px] text-muted-foreground">{formatBytes(resultItem.output_size)}</span> : null}
                <span className="font-mono text-[11px] text-muted-foreground">{formatTokens(resultItem.target_tokens)} tokens</span>
              </div>
            ...
            <span className="font-mono text-xs text-amber-400 animate-pulse">Converting...</span>
          ) : (
            <span className="font-mono text-xs text-muted-foreground/60 italic">Awaiting convert</span>
          )}
        </div>
      ) : null}
    </div>
