...
import type { ConfigResponse, HealthResponse, SamplesResponse } from "./types";
...
export function parseError(status: number, text: string): ApiError {
  ...
  if (errCode && typeof errCode === "string") {
    return new ApiError(typeof body?.message === "string" ? body.message : `HTTP ${status}`, classifyError(errCode), status, body ?? undefined);
  }
  return new ApiError(body?.message ? body.message : `HTTP ${status}`, "unknown", status, body ?? undefined);
}
